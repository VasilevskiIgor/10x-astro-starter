...
import { RulesBuilderService, CostLevel, TimeSlot, type RulesConfig } from "@/services/rules-builder.service";
...
describe("RulesBuilderService", () => {
  ...
  describe("generateRulesContent()", () => {
    ...
    it("should include budget-specific guidance when budget is provided", () => {
      ...
      expect(budgetResult).toContain("Focus on free/cheap activities, local food, public transport");

      expect(luxuryResult).toContain("Preferred budget level: $$$$");
      expect(luxuryResult).toContain("Exclusive experiences, fine dining, private tours");
    });
  ...
  describe("Input Validation and Edge Cases", () => {
    ...
    it("should throw error for invalid trip duration (negative)", () => {
      expect(() => service.generateRulesContent(-5)).toThrow("Invalid trip duration");
    });
  ...
  describe("validateActivityCount()", () => {
    ...
    it("should provide warning for minimum activity count", () => {
      ...
      expect(result.warnings[0]).toContain("Consider adding more activities for better experience");
    });
  ...
  describe("validateActivityDuration()", () => {
    ...
    it("should provide warning for very long activities (>4 hours)", () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0]).toContain("Consider splitting long activities into multiple parts");
    });
  ...
  describe("validateTimeProgression()", () => {
    ...
    it("should detect invalid time format in activities", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.violations).toContainEqual(expect.stringContaining("Invalid time format"));
    });
